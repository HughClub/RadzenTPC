...
{
    public partial class EditNationComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic n_nationkey { get; set; }
        ...
        {
            var tpcHGetNationBynNationkeyResult = await TpcH.GetNationBynNationkey(n_nationkey);
            nation = tpcHGetNationBynNationkeyResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Nation args)
        {
            try
            {
                var tpcHUpdateNationResult = await TpcH.UpdateNation(n_nationkey, nation);
                DialogService.Close(nation);
            }
            catch (System.Exception tpcHUpdateNationException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Nation" });
            }
        }
