...
{
    public partial class EditLineitemComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic l_orderkey { get; set; }

        [Parameter]
        public dynamic l_linenumber { get; set; }
        ...
        {
            var tpcHGetLineitemByLOrderkeyAndLLinenumberResult = await TpcH.GetLineitemByLOrderkeyAndLLinenumber(l_orderkey, l_linenumber);
            lineitem = tpcHGetLineitemByLOrderkeyAndLLinenumberResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Lineitem args)
        {
            try
            {
                var tpcHUpdateLineitemResult = await TpcH.UpdateLineitem(l_orderkey, l_linenumber, lineitem);
                DialogService.Close(lineitem);
            }
            catch (System.Exception tpcHUpdateLineitemException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Lineitem" });
            }
        }
