...
namespace RadzenDb.Pages
{
    public partial class EditSupplierComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic s_suppkey { get; set; }
        ...
        {
            var tpcHGetSupplierBysSuppkeyResult = await TpcH.GetSupplierBysSuppkey(s_suppkey);
            supplier = tpcHGetSupplierBysSuppkeyResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Supplier args)
        {
            try
            {
                var tpcHUpdateSupplierResult = await TpcH.UpdateSupplier(s_suppkey, supplier);
                DialogService.Close(supplier);
            }
            catch (System.Exception tpcHUpdateSupplierException)
            {
