...
{
    public partial class EditPartComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic p_partkey { get; set; }
        ...
        {
            var tpcHGetPartBypPartkeyResult = await TpcH.GetPartBypPartkey(p_partkey);
            part = tpcHGetPartBypPartkeyResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Part args)
        {
            try
            {
                var tpcHUpdatePartResult = await TpcH.UpdatePart(p_partkey, part);
                DialogService.Close(part);
            }
            catch (System.Exception tpcHUpdatePartException)
            {
