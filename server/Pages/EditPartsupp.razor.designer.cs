...
{
    public partial class EditPartsuppComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic ps_partkey { get; set; }

        [Parameter]
        public dynamic ps_suppkey { get; set; }
        ...
        {
            var tpcHGetPartsuppByPsPartkeyAndPsSuppkeyResult = await TpcH.GetPartsuppByPsPartkeyAndPsSuppkey(ps_partkey, ps_suppkey);
            partsupp = tpcHGetPartsuppByPsPartkeyAndPsSuppkeyResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Partsupp args)
        {
            try
            {
                var tpcHUpdatePartsuppResult = await TpcH.UpdatePartsupp(ps_partkey, ps_suppkey, partsupp);
                DialogService.Close(partsupp);
            }
            catch (System.Exception tpcHUpdatePartsuppException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Partsupp" });
            }
        }
