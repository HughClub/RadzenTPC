...
namespace RadzenDb.Pages
{
    public partial class EditRegionComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic r_regionkey { get; set; }
        ...
        {
            var tpcHGetRegionByrRegionkeyResult = await TpcH.GetRegionByrRegionkey(r_regionkey);
            region = tpcHGetRegionByrRegionkeyResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(RadzenDb.Models.TpcH.Region args)
        {
            try
            {
                var tpcHUpdateRegionResult = await TpcH.UpdateRegion(r_regionkey, region);
                DialogService.Close(region);
            }
            catch (System.Exception tpcHUpdateRegionException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Region" });
            }
        }
